...
def label_contains_jetski(label_path):
    with open(label_path, "r") as f:
        for line in f:
            if line.strip() and int(line.split()[0]) == JETSki_CLASS_ID:
                return True
    return False

def copy_jetski_split(images_dir, labels_dir):
    copied = 0

    for lbl_path in labels_dir.glob("*.txt"):
        if not label_contains_jetski(lbl_path):
            continue
...
val_copied = copy_jetski_split(VAL_IMAGES, VAL_LABELS)
test_copied = copy_jetski_split(TEST_IMAGES, TEST_LABELS)

print(f"Copied {val_copied} jetski samples from VAL to TRAIN")
print(f"Copied {test_copied} jetski samples from TEST to TRAIN")
print(f"Total jetski samples copied: {val_copied + test_copied}")