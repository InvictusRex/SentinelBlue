...
def main():
    ...
    parser.add_argument("--web-port", type=int, default=8000, help="Optional Pi mission API/static dashboard port")
    parser.add_argument("--radio-rate", type=float, default=5.0, help="NRF24 broadcast rate in Hz (default: 5.0)")
    parser.add_argument("--no-web", action="store_true", help="Disable the optional Pi mission API/static dashboard server")
    ...
    print(f"[*] Mission API:             {'DISABLED' if args.no_web else f'ENABLED on :{args.web_port}'}")
    ...
    # 3. Existing mission planner/API retained for operators that use /api/mission/*.
    grid_module = GridSearchModule(mav_manager)
    web_module = None
    if args.no_web:
        print("[*] Mission API: Disabled by --no-web.")
    else:
        web_module = WebDashboardModule(
    ...
    if web_module:
        print(f"  • Mission API:                http://<pi-ip>:{args.web_port}/api/mission/status")
    print("=" * 76 + "\n")
