...
class WebDashboardModule:
    ...
    def start(self):
        ...
        print(f"[✓] Module 2 (Mission API): HTTP/API server live at http://0.0.0.0:{self.port}")
    ...
    def _create_handler(self):
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                ...
                if not os.path.exists(os.path.join(WEB_DIST_DIR, "index.html")):
                    self.send_error(503, "Build telemetry/live-web-dashboard with npm run build before serving static dashboard files.")
                    return

                relative_path = "index.html" if clean_path in ("/", "/index.html") else clean_path.lstrip("/")
                target_file = os.path.abspath(os.path.join(WEB_DIST_DIR, relative_path))
                if not target_file.startswith(WEB_DIST_DIR + os.sep):
                    self.send_error(403, "Forbidden")
                    return
                if not os.path.exists(target_file) or os.path.isdir(target_file):
                    target_file = os.path.join(WEB_DIST_DIR, "index.html")
